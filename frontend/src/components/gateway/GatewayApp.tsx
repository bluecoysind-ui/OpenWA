...
export function GatewayApp() {
  ...
  if (authNeeded) return <OpenWALogin />;
  ...
  if (nav === "tools") {
    return (
      <div className="glass scroll-thin min-w-0 flex-1 overflow-auto rounded-2xl p-4">
        <h2 className="mb-4 text-base font-semibold">Gateway Tools</h2>
        <ToolsPanel />
      </div>
    );
  }
  if (nav === "settings") return <SettingsHub />;
...
function Directory({ title, kind }: { title: string; kind: "dm" | "group" }) {
  ...
  return (
    <div className="glass scroll-thin min-w-0 flex-1 overflow-auto rounded-2xl p-4">
      ...
      {rows.length === 0 && chatsLoading ? (
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }, (_, i) => (
            <DirectorySkeleton key={i} />
          ))}
        </div>
      ) : rows.length === 0 ? (
        <EmptyState>
...
function DirectorySkeleton() {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-line bg-night/30 p-3">
      <div className="size-11 shrink-0 animate-pulse rounded-full bg-white/10" />
    ...
    <section className="glass flex h-full flex-col rounded-2xl">
      ...
      <div className="scroll-thin min-h-0 flex-1 overflow-y-auto px-2 pb-3">
        {filtered.length === 0 && chatsLoading ? (
          ...
          <EmptyState>{emptyChatsMessage(chats.length, query)}</EmptyState>
        ) : (
          filtered.map((c) => (
            <button
              key={c.id}
...
function ChatRowSkeleton() {
  return (
    <div className="mb-0.5 flex items-center gap-3 px-2.5 py-2.5">
      <div className="size-11 shrink-0 animate-pulse rounded-full bg-white/10" />
      <div className="min-w-0 flex-1 space-y-2">
        <div className="h-3 w-2/3 animate-pulse rounded bg-white/10" />
        <div className="h-2.5 w-1/2 animate-pulse rounded bg-white/8" />
...
function ChatAvatar({ chat, size = 44, pictureUrl }: { chat: ChatPreview; size?: number; pictureUrl?: string | null }) {
  const [broken, setBroken] = useState(false);
  ...
  return (
    <div className="relative shrink-0 overflow-hidden rounded-full bg-white/10" style={{ width: size, height: size }}>
      {showPhoto ? (
        <img src={photo} alt="" className="size-full object-cover" onError={() => setBroken(true)} />
      ) : chat.avatar === "initials" || !chat.icon ? (
...
function MessageSkeleton({ mine }: { mine: boolean }) {
  return (
    <div className={cn("flex max-w-[68%] flex-col", mine ? "self-end" : "self-start")}>
      <div className={cn("h-12 animate-pulse rounded-2xl", mine ? "w-48 rounded-br-sm bg-wa/30" : "w-56 rounded-bl-sm bg-white/10")} />
      <div className={cn("mt-1 h-2 w-10 animate-pulse rounded bg-white/8", mine && "self-end")} />
    </div>
  );
}
