...
import { useTemplatesQuery } from "@/lib/openwa-query";
import { cn } from "@/lib/cn";
import { sessionDisplayName, sessionInitials } from "@/lib/openwa/sessionLabel";
...
function TemplatesModal() {
  ...
  return (
    <Card title="Message Templates" sub="From the gateway API for the active session">
      <div className="space-y-2 text-left">
        {templates.isLoading ? <p className="text-sm text-muted">Loading…</p> : null}
        {rows.length === 0 && !templates.isLoading ? (
          <p className="text-sm text-muted">No templates — create them in Settings → Templates.</p>
        ) : null}
        {rows.map((t) => (
          <button
            key={t.id}
...
function SearchModal() {
  ...
  return (
    <Card title="Search" sub="Gateway message index (GET /search)" wide>
      <GlobalSearch
    ...
    <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="space-y-2">
            {sessions.map((s) => (
              <div key={s.sessionId} className="flex items-center gap-3 rounded-xl border border-line bg-night/30 p-3">
                <div className="grid size-10 place-items-center overflow-hidden rounded-full bg-gradient-to-br from-wa to-wa-deep text-sm font-bold text-night">
                  {sessionInitials(sessionDisplayName(s))}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-medium">{sessionDisplayName(s)}</div>
      ...
      <div className="space-y-4">
        ...
        <section className="glass rounded-2xl p-4 text-left">
          <h3 className="mb-3 text-sm font-semibold">OpenWA credentials</h3>
          <input
            value={url}
            onChange={(e) => setUrl(e.target.value)}
          ...
          <div className="flex gap-2">
            <button className="rounded-xl bg-wa px-3 py-2 text-xs font-semibold text-night" onClick={() => void login("operator", "", apiKey, url)}>
              Connect backend
            </button>
