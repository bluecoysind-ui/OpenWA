...
- **Bot config:** access mode + lists, prefix, commands on/off, `autoRead`, `alwaysOnline`, welcome on `group.join` (paced `sendText`), sticker pack/author for `#sticker` media converts. Commands are a core module (`BOT_COMMANDS` + per-session enabled). `#sticker` accepts a URL, a captioned image/video, or a reply to image/video (no remove.bg). Size cap is enforced before ffmpeg; conversion uses `mediaConversion.timeoutMs`; per-sender cooldown holds under concurrent senders. ffmpeg concurrency default 2 (hard max 4) with a 2-slot wait queue; saturation is REST `429` / `#sticker` “busy, try again”.
